// Amazon Spending Analysis / Ausgaben-Analyse – Dashboard 5.8 (Chrome)
...
// Stored in chrome.storage.local as array of {w, s, c} where c = category key
var USER_KW_STORAGE_KEY='amz-user-keywords';
var userKeywords=[];

function loadUserKeywords(cb){
  chrome.storage.local.get([USER_KW_STORAGE_KEY], function(r) {
    try { if(r[USER_KW_STORAGE_KEY]) userKeywords = r[USER_KW_STORAGE_KEY]; else userKeywords = []; } catch(e) { userKeywords = []; }
    if(cb) cb();
  });
}
function saveUserKeywords(){
  var obj = {}; obj[USER_KW_STORAGE_KEY] = userKeywords;
...
var itemMultiSelect=false, selectedItemKeys={};

// ─── STORAGE ─────────────────────────────────────────────
// ─── STORAGE (chrome.storage.local) ─────────────────────
function loadOrders(cb){
  chrome.storage.local.get(['amz-orders-v2','amzOrders',USER_KW_STORAGE_KEY], function(r) {
    try { if(r['amz-orders-v2']) orders = r['amz-orders-v2']; } catch(e) {}
    // Auto-sync: merge orders scraped by content.js (stored under 'amzOrders')
    if(r['amzOrders'] && Array.isArray(r['amzOrders']) && r['amzOrders'].length > 0) {
      var ids = {};
      orders.forEach(function(o) { ids[o.id] = true; });
      var added = 0;
      r['amzOrders'].forEach(function(o) {
        if(!ids[o.id]) { orders.push(o); ids[o.id] = true; added++; }
      });
      if(added > 0) {
        orders.sort(function(a, b) { return b.date.localeCompare(a.date); });
        chrome.storage.local.set({ 'amz-orders-v2': orders, 'amzOrders': [] });
      }
    }
    try { if(r[USER_KW_STORAGE_KEY]) userKeywords = r[USER_KW_STORAGE_KEY]; else userKeywords = []; } catch(e) { userKeywords = []; }
    recategorizeAll();
...
function saveOrders(){
  chrome.storage.local.set({ 'amz-orders-v2': orders });
}
...
function render(){
  ...
  if(orders.length===0){
    ...
    // Theme toggle on empty state
    var themeBtnE=document.getElementById('theme-toggle');
    var curThemeE=localStorage.getItem('amz-theme')||'dark';
    document.documentElement.setAttribute('data-theme',curThemeE);
    if(themeBtnE){
      themeBtnE.textContent=curThemeE==='dark'?'\u2600\uFE0F':'\uD83C\uDF19';
      themeBtnE.addEventListener('click',function(){
        var n=curThemeE==='dark'?'light':'dark';
        localStorage.setItem('amz-theme',n);
        document.documentElement.setAttribute('data-theme',n);
        curThemeE=n;
        themeBtnE.textContent=n==='dark'?'\u2600\uFE0F':'\uD83C\uDF19';
      });
    }
    document.getElementById('app').innerHTML=
      '<div style="text-align:center;padding:60px 24px;max-width:480px;margin:0 auto">'+
      '<div style="font-size:64px;margin-bottom:16px">\uD83D\uDCCA</div>'+
      '<h1 style="font-size:24px;font-weight:800;margin-bottom:8px;font-family:var(--mono)">'+t('emptyTitle')+' 5.8</h1>'+
  ...
  // Nav
  var navHtml='';
  [['dashboard',t('dashboard')],['orders',t('orders')],['items',t('itemsView')],['import',t('import')]].forEach(function(p){
  ...
  var curTheme=localStorage.getItem('amz-theme')||'dark';
  document.documentElement.setAttribute('data-theme',curTheme);
  if(themeBtn){
    themeBtn.textContent=curTheme==='dark'?'\u2600\uFE0F':'\uD83C\uDF19';
    themeBtn.title=curTheme==='dark'?'Light Mode':'Dark Mode';
    themeBtn.addEventListener('click',function(){
      var next=curTheme==='dark'?'light':'dark';
      localStorage.setItem('amz-theme',next);
  ...
  if(logoEl) logoEl.innerHTML=t('appTitle')+' <span style="font-size:10px;color:var(--t3);font-weight:400">5.8</span>';
  document.title=t('analysis')+' 5.8';
  ...
  // ─── ITEMS ─────────────────────────────────────────────
  if(view==='items'){
    // Flatten all items from filtered orders with stable keys
    var allItems=[];
    a.filtered.forEach(function(o){
      o.items.forEach(function(it,idx){
        allItems.push({name:it.name,price:it.price,qty:it.qty,category:it.category,date:o.date,orderId:o.id,itemIdx:idx,total:it.price*it.qty,_userCat:it._userCat,key:o.id+'|'+idx});
      });
    });

    // Apply category filter
    if(filterCats.length>0) allItems=allItems.filter(function(it){return filterCats.indexOf(it.category)>=0;});
    // Apply month filter
    if(filterMonths.length>0) allItems=allItems.filter(function(it){var mo=parseInt(it.date.slice(5,7));return filterMonths.indexOf(mo)>=0;});
    // Apply search
    if(search){var s=search.toLowerCase();allItems=allItems.filter(function(it){return it.name.toLowerCase().indexOf(s)>=0||it.date.indexOf(s)>=0||it.orderId.indexOf(s)>=0;});}

    // Sort by price descending
    allItems.sort(function(ab,bb){return bb.total-ab.total;});

    var MNAMES=t('months');
    var iSelCount=0;for(var isk in selectedItemKeys){if(selectedItemKeys[isk])iSelCount++;}
    ...
    var flTotal=allItems.reduce(function(s,it){return s+it.total;},0);
    html+='<div style="display:flex;align-items:center;gap:8px;font-size:11px;color:var(--t3);margin-bottom:12px;flex-wrap:wrap">';
    html+='<button class="filter-btn" id="toggle-item-multiselect" style="background:'+(itemMultiSelect?'rgba(76,158,235,0.2)':'var(--bg3)')+';color:'+(itemMultiSelect?'#4C9EEB':'var(--t3)')+';font-size:10px;padding:3px 10px">\u2611 '+t('multiSelect')+'</button>';
    html+='<span>'+allItems.length+' '+t('itemsCount')+' \u00B7 '+fmtE(flTotal)+(hasFilters?' ('+t('filtered')+')':'')+'</span>';
    html+=exportButtonsHTML('items');
    html+='</div>';

    // Bulk action bar
    if(itemMultiSelect && iSelCount>0){
      html+='<div id="item-bulk-bar" style="position:sticky;top:0;z-index:50;background:var(--bg2);border:1px solid var(--accent);border-radius:10px;padding:10px 14px;margin-bottom:12px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">';
      html+='<span style="font-size:12px;font-weight:700;color:var(--accent)">'+iSelCount+' '+t('selected')+'</span>';
      html+='<span style="font-size:11px;color:var(--t3)">\u2192 '+t('assignTo')+':</span>';
      CK.forEach(function(k){
        if(k==='sonstiges')return;
        var c=CATS[k];
        html+='<button class="filter-btn item-bulk-cat" data-ibulkcat="'+k+'" style="padding:2px 8px;font-size:10px;background:'+c.color+'15;color:'+c.color+';border-color:'+c.color+'30">'+c.icon+' '+catLabel(k)+'</button>';
      });
      html+='<button class="filter-btn" id="item-bulk-deselect" style="margin-left:auto;padding:2px 8px;font-size:10px;background:rgba(232,93,117,0.15);color:#E85D75">\u2715 '+t('deselectAll')+'</button>';
      html+='</div>';
    }

    // Select all visible
    if(itemMultiSelect){
      var allKeys=allItems.map(function(it){return it.key;});
      var allChecked=allKeys.length>0&&allKeys.every(function(k){return selectedItemKeys[k];});
      html+='<div style="margin-bottom:8px;display:flex;gap:8px;align-items:center">';
      html+='<label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--t3);cursor:pointer"><input type="checkbox" id="select-all-items" '+(allChecked?'checked':'')+' style="cursor:pointer"> '+t('selectAllSingle')+' ('+allItems.length+')</label>';
      html+='</div>';
    }
    ...
    allItems.forEach(function(it){
      var cat=CATS[it.category]||CATS.sonstiges;
      var isChecked=!!selectedItemKeys[it.key];
      html+='<div class="card" style="padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;'+(isChecked?'outline:2px solid var(--accent);outline-offset:-2px;':'')+'" data-itemkey="'+esc(it.key)+'" data-item-oid="'+esc(it.orderId)+'">';
      if(itemMultiSelect){
        html+='<input type="checkbox" class="item-ms-check" data-imskey="'+esc(it.key)+'" '+(isChecked?'checked':'')+' style="cursor:pointer;width:16px;height:16px;flex-shrink:0">';
      }
      html+='<span style="font-size:14px;flex-shrink:0">'+cat.icon+'</span>';
      html+='<div style="flex:1;min-width:0">';
      html+='<div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(it.name)+'</div>';
      html+='<div style="font-size:10px;color:var(--t3);display:flex;gap:8px;flex-wrap:wrap;margin-top:2px">';
      html+='<span>'+fmtD(it.date)+'</span>';
      html+='<span style="background:'+cat.color+'15;color:'+cat.color+';padding:0 5px;border-radius:3px">'+catLabel(it.category)+'</span>';
      if(it.qty>1) html+='<span>\u00D7'+it.qty+'</span>';
      html+='</div></div>';
      html+='<div style="font-family:var(--mono);font-weight:700;font-size:13px;color:var(--accent);flex-shrink:0;text-align:right">'+fmtE(it.total)+'</div>';
      html+='</div>';
    });
    ...
    // Multiselect toggle
    document.getElementById('toggle-item-multiselect').addEventListener('click',function(){
      itemMultiSelect=!itemMultiSelect;
      if(!itemMultiSelect) selectedItemKeys={};
      render();
    });

    // Checkboxes
    document.querySelectorAll('.item-ms-check').forEach(function(cb){
      cb.addEventListener('click',function(e){e.stopPropagation();});
      cb.addEventListener('change',function(){
        var k=cb.getAttribute('data-imskey');
        if(cb.checked) selectedItemKeys[k]=true;
        else delete selectedItemKeys[k];
        render();
      });
    });

    // Click on card toggles selection in multiselect mode, opens modal otherwise
    document.querySelectorAll('[data-itemkey]').forEach(function(card){
      card.addEventListener('click',function(e){
        if(e.target.tagName==='INPUT') return;
        if(itemMultiSelect){
          var k=card.getAttribute('data-itemkey');
          if(selectedItemKeys[k]) delete selectedItemKeys[k];
          else selectedItemKeys[k]=true;
          render();
        } else {
          selectedOrderId=card.getAttribute('data-item-oid');
          renderModal();
        }
      });
    });
    ...
    // Select all
    var selAllItems=document.getElementById('select-all-items');
    if(selAllItems){
      selAllItems.addEventListener('change',function(){
        allItems.forEach(function(it){
          if(selAllItems.checked) selectedItemKeys[it.key]=true;
          else delete selectedItemKeys[it.key];
        });
        render();
      });
    }
    ...
    // Bulk category assignment
    document.querySelectorAll('[data-ibulkcat]').forEach(function(btn){
      btn.addEventListener('click',function(){
        var newCat=btn.getAttribute('data-ibulkcat');
        var changed=0;
        for(var k in selectedItemKeys){
          if(!selectedItemKeys[k]) continue;
          var parts=k.split('|');
          var oid=parts[0], iIdx=parseInt(parts[1]);
          for(var i=0;i<orders.length;i++){
            if(orders[i].id===oid){
              orders[i].items[iIdx].category=newCat;
              orders[i].items[iIdx]._userCat=true;
              changed++;
              break;
            }
          }
    ...
    // Deselect all
    var iDeselBtn=document.getElementById('item-bulk-deselect');
    if(iDeselBtn) iDeselBtn.addEventListener('click',function(){selectedItemKeys={};render();});

    // Export handlers
    document.getElementById('items-excel').addEventListener('click',function(){exportItemsExcel(allItems);});
    document.getElementById('items-pdf').addEventListener('click',function(){exportItemsPDF(allItems);});
  }
...
function drawDonut(data){
  ...
  function paint(){
    ...
    if(hovIdx>=0){
      ...
      ctx.fillStyle=tc('--t2');ctx.font='400 10px "DM Sans",sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
      ctx.fillText(h.label,cx,cy-14);
      ctx.fillStyle=tc('--t1');ctx.font='800 '+valSize+'px "DM Mono",monospace';
      ...
      ctx.fillStyle=tc('--t1');ctx.font='800 '+totSize+'px "DM Mono",monospace';
...
function drawMonthlyBars(monthlyData){
  ...
  monthlyData.forEach(function(d,i){
    ...
    // Value above bar
    if(d.total>50){
      ctx.fillStyle=tc('--t2');ctx.font='9px "DM Mono",monospace';ctx.textAlign='center';ctx.textBaseline='bottom';
      ctx.fillText(Math.round(d.total),x+bw/2,y-3);
    }

    // Month label
    ctx.fillStyle=tc('--t3');ctx.font='9px "DM Sans",sans-serif';ctx.textAlign='center';ctx.textBaseline='top';
  ...
  ctx.setLineDash([]);
  ctx.fillStyle=tc('--t3');ctx.font='9px "DM Mono",monospace';ctx.textAlign='right';ctx.textBaseline='bottom';
...
function drawMonthlyLine(monthlyData){
  ...
  // Y-axis labels
  ctx.fillStyle=tc('--t3'); ctx.font='10px "DM Mono",monospace'; ctx.textAlign='right'; ctx.textBaseline='middle';
  for(var i=0;i<=4;i++){
    var val=max*(i/4);
    var y=pad.top+ch-ch*(i/4);
    ctx.fillText(Math.round(val)+' \u20AC',pad.left-8,y);
    ctx.strokeStyle=tc('--t3')+'20'; ctx.lineWidth=1;
  ...
  ctx.beginPath(); ctx.moveTo(pad.left,avgY); ctx.lineTo(W-pad.right,avgY); ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillStyle=tc('--t3'); ctx.font='9px "DM Mono",monospace'; ctx.textAlign='right';
...
function drawYearlyBars(yearlyMap){
  ...
  keys.forEach(function(yr,i){
    ...
    // Value on top
    ctx.fillStyle=tc('--t1'); ctx.font='bold 11px "DM Mono",monospace'; ctx.textAlign='center'; ctx.textBaseline='bottom';
    ctx.fillText(fmtE(vals[i]),x+barW/2,y-4);

    // Year label
    ctx.fillStyle=tc('--t3'); ctx.font='11px "DM Sans",sans-serif'; ctx.textBaseline='top';
...
function exportItemsExcel(items) {
  var rows = [];
  var filterLabel=filterCats.length>0?' ('+filterCats.map(function(k){return catLabel(k);}).join(', ')+')':'';
  rows.push({ cells: [{ value: t('appTitle') + ' ' + t('itemsView') + ' ' + getExportTitle() + filterLabel, style: 'title' }] });
  rows.push({ cells: [{ value: items.length + ' ' + t('itemsCount') }] });
  rows.push({ cells: [] });
  rows.push({ cells: [{ value: t('itemName'), style: 'hdr' }, { value: t('category'), style: 'hdr' }, { value: t('orderDate'), style: 'hdr' }, { value: t('quantity'), style: 'hdr' }, { value: t('price'), style: 'hdr' }] });
  items.forEach(function(it) {
    rows.push({ cells: [{ value: it.name }, { value: catLabel(it.category) }, { value: fmtD(it.date) }, { value: it.qty, type: 'Number' }, { value: it.total, type: 'Number', style: 'num' }] });
  });
  var sumTotal=items.reduce(function(s,it){return s+it.total;},0);
  rows.push({ cells: [{ value: '' }, { value: '' }, { value: '' }, { value: t('totalSum'), style: 'subtitle' }, { value: sumTotal, type: 'Number', style: 'numBold' }] });
  var sheetName = LANG === 'en' ? 'Items' : 'Artikel';
  downloadExcel([
    { name: sheetName, rows: rows, colWidths: [55, 20, 14, 8, 14], freezeRow: 4 }
  ], 'Amazon-' + sheetName + '-' + getExportTitle() + '.xlsx');
}

function exportItemsPDF(items) {
  var filterLabel=filterCats.length>0?' ('+filterCats.map(function(k){return catLabel(k);}).join(', ')+')':'';
  var title = t('appTitle') + ' ' + t('itemsView') + ' \u2013 ' + getExportTitle() + filterLabel;
  var html = '<h1>\uD83D\uDCE6 ' + esc(title) + '</h1>';
  html += '<h2>' + items.length + ' ' + t('itemsCount') + '</h2>';
  html += '<table><tr><th>' + t('itemName') + '</th><th>' + t('category') + '</th><th>' + t('orderDate') + '</th><th class="right">' + t('quantity') + '</th><th class="right">' + t('price') + '</th></tr>';
  items.forEach(function(it) {
    var cat=CATS[it.category]||CATS.sonstiges;
    html += '<tr><td>' + esc(it.name.slice(0, 55)) + '</td><td>' + cat.icon + ' ' + esc(catLabel(it.category)) + '</td><td>' + fmtD(it.date) + '</td><td class="right">' + it.qty + '</td><td class="right mono">' + fmtE(it.total) + '</td></tr>';
  });
  var sumTotal=items.reduce(function(s,it){return s+it.total;},0);
  html += '<tr style="font-weight:bold;border-top:2px solid #2D9D78"><td colspan="4" style="text-align:right;padding-top:8px">' + t('totalSum') + '</td><td class="right mono" style="padding-top:8px">' + fmtE(sumTotal) + '</td></tr>';
  html += '</table>';
...
// ─── INIT (async: first load language, then orders, then render) ──
