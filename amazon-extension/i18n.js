...
var LANG = (navigator.language || '').startsWith('en') ? 'en' : 'de';
...
function setLang(newLang) {
  LANG = newLang;
  chrome.storage.local.set({ 'amz-lang': LANG });
}

function initLang() {
  return new Promise(function(resolve) {
    chrome.storage.local.get(['amz-lang'], function(r) {
      if (r['amz-lang']) LANG = r['amz-lang'];
      resolve();
    });
  });
}
